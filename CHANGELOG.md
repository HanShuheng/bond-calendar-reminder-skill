...
- 新增 `is-trade-day` 命令，用于查询某个日期是否为中国 A 股交易日。
- 新增 `trade_calendar_strategy` 配置和交易日历适配器协议，支持用户接入自己的 Python 交易日历适配器。
- 新增内置 BaoStock 交易日历示例适配器，并将 `baostock` 加入依赖。

### 文档

- 更新 README、SKILL 和数据适配器协议文档，补充交易日查询能力和配置入口。
